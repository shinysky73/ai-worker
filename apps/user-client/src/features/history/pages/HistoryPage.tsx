    ...
    return (
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-2xl p-6 text-white">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-indigo-200">
            ...
            <span className="text-indigo-300 text-xs">{new Date(selectedItem.createdAt).toLocaleString('ko-KR')}</span>
          </div>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">스크립트 히스토리</h1>
          {total > 0 && <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">총 {total}개</p>}
        </div>
        <Link
          to="/"
          className="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors"
        >
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {loading && (
        <div className="text-center py-16">
          <div className="animate-spin h-8 w-8 mx-auto border-[3px] border-indigo-500 border-t-transparent rounded-full" />
      ...
      {!loading && !error && items.length === 0 && (
        <div className="text-center py-20">
          ...
          <Link to="/" className="inline-flex items-center gap-1.5 mt-4 text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
            첫 스크립트 생성하기 →
          </Link>
        </div>
      )}
        ...
        <>
          <div className="space-y-2">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-200 dark:hover:border-indigo-700 hover:shadow-sm transition-all duration-150"
              >
      ...
      {detailLoading && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="animate-spin h-8 w-8 border-[3px] border-indigo-500 border-t-transparent rounded-full" />
        </div>
      )}
    </div>
  );
